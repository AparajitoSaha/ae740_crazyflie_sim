...
class SelfAdaptiveMPC:
    ...
    def generate_mpc(self):
        ...
        Q = np.diag([
            1.5, 1.5, 3.0,   # positions (increased pz weight to track altitude better)
    ...
    def update_step(self, dt, x_now, target_now):
        ...
        if self.predictor_type=='single_learner':
            ...
            # Construct feature vector: [pursuer_state, pursuer_control, target_memory]
            # pursuer state (9) + control (4) + target memory (3*memory_horizon)
            feature_vector = np.concatenate([x_in, u_in, target_memory_vector])
            z = self.Bz @ feature_vector  # Map to feature space using input mask
            
            # Compute random features: rf = (1/sqrt(n_rf)) * cos(omega @ z + b)
            omega_z_plus_b = self.omega @ z + self.b.flatten()
            rf = (1.0 / np.sqrt(self.n_rf)) * np.cos(omega_z_plus_b)  # shape: (n_rf,)
            
            # Predict current target position using learned model
            # Prediction: y_pred = Bh @ (alpha @ rf)
            # alpha shape: (len(target_mask), n_rf), rf shape: (n_rf,)
            y_pred = self.Bh @ (alpha_in @ rf)  # shape: (3,)
    ...
    def get_prediction_error(self):
        ...
        if self.predictor_type=='single_learner':
            ...
            # Use last known pursuer state and control for prediction (approximation)
            x_pred = self.x_last if self.x_last is not None else np.zeros(9)
            u_pred = self.u_last if self.u_last is not None else np.zeros(4)

            # iterate over mpc horizon
            for i in range(self.num_steps):
                # Construct feature vector for prediction
                feature_vector = np.concatenate([x_pred, u_pred, past_memory_vector])
                ...
                target_next = self.Bh @ (self.alpha_last @ rf)
                
                predicted_traj[:,i+1] = target_next
                # Get actual target position from log (one step ahead from past)
                actual_idx = int(self.num_steps*self.step_size) - (i+1)*int(self.step_size)
            ...
            # compute error as difference in increments (velocity error)
            pred_increments = predicted_traj[:, 1:] - predicted_traj[:, :-1]
            actual_increments = actual_traj[:, 1:] - actual_traj[:, :-1]
            vel_error_matrix = (pred_increments - actual_increments) / self.mpc_dt  
            overall_rms_error = np.sqrt(np.mean(vel_error_matrix**2))  # scalar that depicts how much error in velocity prediction over the mpc horizon
        else:
            if self.predictor_type=='multiple_learners':
                raise NotImplementedError('multiple_learners predictor type not implemented yet')
            else:
    ...
    def solve_mpc(self, x0, target_now, dt):
        ...
        if self.predictor_type=='single_learner':
            ...
            for i in range(N):
                ...
                yref[0:3, i] = target_next  # predicted target position
                if i == 0:
                    # For first step, use current target velocity if available, otherwise approximate
                    yref[3:6, i] = (target_next - target_now[:3]) / self.mpc_dt
                else:
                ...
                # Update memory vector: shift by 3 (remove oldest) and add latest predicted position
                target_memory_vector = np.roll(target_memory_vector, -3)
                target_memory_vector[-3:] = target_next
                
            # Final predicted target state
            yref_e = np.zeros(nx)
            ...
            if self.predictor_type=='multiple_learners':
                ...
                # Fallback: use current target position as reference
                yref = np.zeros((nx, N))
                for i in range(N):
                    yref[0:3, i] = target_now[:3]
                yref_e = np.zeros(nx)
                yref_e[0:3] = target_now[:3]
